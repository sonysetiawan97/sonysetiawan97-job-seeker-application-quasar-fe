        ...
        >
          <div v-if="['QSelect'].indexOf(field.type) >= 0">
            <component
              ...
              v-on="field.events"
              :hint="
                !v$.form[field.name].required
                  ? `-- optional -- ${field.props.hint}`
                  : `${field.props.hint}`
              "
          ...
          <div
            v-else-if="['QRadio'].indexOf(field.type) >= 0"
          ...
          <div v-else-if="['QFile'].indexOf(field.type) >= 0">
            ...
            <img
              v-if="
                ['show', 'update', 'trashed'].indexOf(stateForm) >= 0 &&
                field.name
              "
            ...
            >
              <template v-slot:file="{ file }">
                <q-chip class="full-width q-my-md" square>
                  <q-avatar>
...
import { QInput, QSelect, QFile, QRadio, QEditor, QToggle } from "quasar";
...
export default {
        ...
        for (const field of fields) {
          ...
          if (type === "QSelect" && reference) {
            events.filter = (search, update, abort) => {
              ...
              this.$store
                .dispatch(`${reference}/fetch`, { params })
                .then((response) => {
                  const { data } = response;
                  const { props } = field;
                  update(() => {
      ...
      for (const fields of this.layout) {
        for (const field of fields) {
          ...
          if (type === "QSelect" && reference) {
            ...
            this.$store
              .dispatch(`${reference}/detail`, {
                id: this.form[field["name"]],
                params,
              })
              .then((response) => {
                const { data } = response;
  ...
  computed: {
    ...
    rulesPassword() {
      return this.stateForm === "create" ? { required } : {};
    },
  },
  validations() {
    return {
